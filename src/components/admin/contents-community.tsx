...
function LiveChat({ postId }: { postId: string }) {
  const qc = useQueryClient();
  const { user } = useAuth();
  ...
  const userIds = Array.from(new Set((messagesQ.data ?? []).map((m) => m.user_id)));
  const profilesQ = useQuery({
    queryKey: ["live-chat-profiles", userIds.sort().join(",")],
  ...
  const send = useMutation({
    ...
    onSuccess: () => {
      setText("");
      qc.invalidateQueries({ queryKey: ["live-chat", postId] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <div className="border-t bg-gradient-to-b from-pink-500/5 to-transparent">
      ...
      <div className="px-4 py-2 border-t space-y-2">
        ...
        <form
          onSubmit={(ev) => { ev.preventDefault(); if (text.trim()) send.mutate({ body: text.trim() }); }}
          className="flex items-center gap-2"
        >
          <Input
            value={text}
            ...
            maxLength={300}
          />
          <Button type="submit" size="icon" className="h-9 w-9" disabled={!text.trim() || send.isPending}>
